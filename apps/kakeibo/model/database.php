<?php
  ...
  class UsersTable extends Table{
    ...
    public function setUserTable($u,$p){
  ...
  class BalancesTable extends Table{
    ...
    //月の収支を返す
    public function getOneMonthBalances($y,$m,$Uid){
      //$m<10 0埋め
      if($m<10){
        $m = "0".$m;
      }
    ...
    //日の収支を返す
    public function getOneDayBalances($y,$m,$d,$Uid){
      if($m<10){
        $m = "0".$m;
      }if($d<10){
        $d = "0".$d;
      }
      $sql = "SELECT * FROM balance WHERE (user_id = {$Uid} )AND (balance_date = '{$y}-{$m}-{$d}')";
      $stmt = $this -> pdo -> prepare($sql);
      $stmt -> execute();
      $rec = $stmt -> fetch(PDO::FETCH_ASSOC);
      return $rec;
